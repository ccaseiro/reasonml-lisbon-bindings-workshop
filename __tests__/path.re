...
test("join()", () =>
  expect(Mypath.join([|"Users", "davidgomes"|])) |> toEqual("Users/davidgomes")
);

test("normalize()", () =>
  expect(Mypath.normalize("/foo/bar//baz/asdf/quux/..")) |> toEqual("/foo/bar/baz/asdf")
);
...
test("relative()", () =>
  expect(Mypath.relative("/data/orandea/test/aaa", "/data/orandea/impl/bbb")) |> toEqual("../../impl/bbb")
);

test("resolve()", () =>
  expect(Mypath.resolve([|"/foo/bar", "/tmp/file"|])) |> toEqual("/tmp/file")
);
